...
def _uploadMission (self, mission, callback=None, params = None):
    ...
    print ('ya he borrado la misión anterior')
    ...
    '''
    print ('voy a llamar al callback ', params)
    if callback != None:
        if self.id == None:
            if params == None:
                callback()
...
def uploadMission(self,flightPlan, blocking=True, callback=None, params = None):
    if blocking:
        self._uploadMission(flightPlan)
    else:
        missionThread = threading.Thread(target=self._uploadMission, args=[flightPlan, callback, params])
        missionThread.start()
