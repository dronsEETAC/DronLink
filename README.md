...
## 4. Demostradores
En este repositorio se proporcionan tres aplicaciones que demuestran el uso de la librería DronLink en diferentes contestos (modo de comunicación directo, modo de comunicación global y enjambre de drones). Puede ser conveniente ver este video que aclara la cuestión de los modos de comunicación tierra-dron.
VIDEO
...
Se trata de una aplicación que presenta al usuario una interfaz basada en botones que permiten realizar las operaciones básicas de control del dron (conectar, armar, despegar, navegar en diferentes direcciones, aterrizar, etc.).
Utiliza el modo de comunicación directa. Por tanto, es la propia aplicación la que usa la clase Dron para comunicarse con el dron a través de la radio de telemetría.
...
La aplicación también presenta una interfaz con algunos botones. Además, muestra un mapa en el que podemos seguir los movimientos del dron. Además de las operaciones más básicas, se puede crear un plan de vuelo (indicando los waypoints sobre el propio mapa) y ejecutarlo.
En estos vídeos puede verse la aplicación en acción y el aspecto que tiene el código.
